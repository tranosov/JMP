...
Dhours_location=sumC(DC.*(lshC-lswC).*PwwC)./sumC(DC.*PwwC);
Dd_location=sumC(DC.*(commutehC-commutewC).*PwwC)./sumC(DC.*PwwC);
DCi_Pww=sumC(DC.*PwwC);

Dhours0_location=sumC(DC.*(lshC-lswC))./sumC(DC);
Dwork_location=sumC(DC.*(workshC-workswC))./sumC(DC);
Shours=lshC./(lshC+lswC);
Shours(isnan(Shours))=0;
DShours_location=sumC(DC.*Shours)./sumC(DC.*(~isnan((lshC./(lshC+lswC)))));
DCi_DSh=sumC(DC.*(~isnan((lshC./(lshC+lswC)))));
Sx=xhC./(xhC+xwC);
Sx(isnan(Sx))=0;
DSx_location=sumC(DC.*Sx)./sumC(DC.*(~isnan((xhC./(xhC+xwC)))));
DCi_DSx=sumC(DC.*(~isnan((xhC./(xhC+xwC)))));
Dx_location=sumC(DC.*(xhC-xwC))./sumC(DC);

Dhours_suburb=(Dhours_location(2)*DCi_Pww(2)+Dhours_location(3)*DCi_Pww(3))/(DCi_Pww(2)+DCi_Pww(3));
Dd_suburb=(Dd_location(2)*DCi_Pww(2)+Dd_location(3)*DCi_Pww(3))/(DCi_Pww(2)+DCi_Pww(3));

Dhours0_suburb=(Dhours0_location(2)*DCi(2)+Dhours0_location(3)*DCi(3))/(DCi(2)+DCi(3));
Dwork_suburb=(Dwork_location(2)*DCi(2)+Dwork_location(3)*DCi(3))/(DCi(2)+DCi(3));
DShours_suburb=(DShours_location(2)*DCi_DSh(2)+DShours_location(3)*DCi_DSh(3))/(DCi_DSh(2)+DCi_DSh(3));
DSx_suburb=(DSx_location(2)*DCi_DSx(2)+DSx_location(3)*DCi_DSx(3))/(DCi_DSx(2)+DCi_DSx(3));
...
Keys = {'scity_dif', 'wlfp_dif', 'hlfp','whours_pww_dif',...
    ...
    'sdj_dif'};
Keyso =  {'shnmarried_','swnmarried_','sshouseexp','cshouseexp','wagegap_actual','wagegap_hw',...
    'p_gradient_swdjobs',...
